...
### 新機能
- 動画出力時に元動画の音声トラックを保持する音声マージ機能を追加
- 全3エクスポート方式（WebCodecs / FFmpeg / MediaRecorder）に対応
- `-c:a copy` による音声無劣化転送を最優先、失敗時のみAAC再エンコへフォールバック
- 無音動画は検出して音声処理をスキップ
- バッチ出力時はFFmpegインスタンスを再利用してロード時間を節約
